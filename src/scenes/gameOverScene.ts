...
export class GameOverScene extends Phaser.Scene{
    ...
    spawnAnimal(x = 0,y = 0, scale = 1): GameObject{
        ...
        var frame = notused[i];
        var animal = this.matter.add.image(x,y, key, frame, {
            shape: {
                type: 'circle',
                radius: 64
            },
    ...
    create () {
        ...
        var background = this.add.image(this.width * 0.5,this.height * 0.70, 'squareSilhouette', "gorilla.png");
        background.setScale(5);
        background.tint = 0xFF0000;
        background.setAlpha(0.8);


        this.returnToMenu = this.add.bitmapText(this.width * 0.15, this.height * 0.9, "jungle", "Tap here to go to the menu.", 50 * SC);
        ...
        var scoreText = this.add.bitmapText(this.width * 0.5, this.height * 0.25, "jungle", "Score: " + this.score, 150 * SC).setOrigin(0.5,0.5);
        scoreText.setScale(0);
        
        var disableScoreCenter = (function(){
        ...
        var coinsTimeline = this.coinsComponent.smoothChangeScore(this.gainedAnimals, 0.1, timeline);
        ...
        var touchToPlay = this.add.bitmapText(this.width * 0.5, this.height * 0.5, "jungle", "Touch an animal to play with it.",  50 * SC).setOrigin(0.5,0.5);
        ...
        if(this.unlockList.length != 0) {
            var charactersUnlockedText = this.add.bitmapText(this.width / 2, this.height * 0.13, "jungle", (this.unlockList.length > 1 ? "Characters" : "Character")+" unlocked !", 90 * SC).setOrigin(0.5, 0.5);
    ...
    addParticleOnAnimal(animal, key, frame){
        var logoSource = {
            getRandomPoint: (function (vec)
            {
                    ...
                    var pixel =  this.textures.getPixel(x, y, key, frame);
